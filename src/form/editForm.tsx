...
const getTitlelen = function (title: string) {
  let len = 0;
  ...
  useEffect(() => {
    ...
    if (isGrid) {
      ...
      for (let i = 0; i < fields.length; i += 1) {
        ...
        const tmpLen = (item.title && getTitlelen(item.title)) || 0
...
const HyFormItem = observer(function ({ item, labelLen, errs, itemProps, onFieldChange }: { item: any, labelLen?: number, errs?: object, itemProps: any, onFieldChange: Function }) {
  ...
  const { field, title = '', rules, dependencies = [], tip } = item
  const props: { [key: string]: any } = { name: field, label: title, rules, dependencies, tooltip: tip }
  if (help) {
    props.help = help
